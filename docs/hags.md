...
- Settings â†’ System â†’ Display â†’ Graphics â†’ "Default graphics settings"
  â†’ "Hardware-accelerated GPU scheduling"
- or the registry (this project never modifies it)

## Why obs-lite targets HAGS-enabled systems
...
that only works with HAGS disabled is not acceptable. obs-lite is
engineered to behave well with HAGS enabled and is benchmarked in that
configuration.

obs-lite **cannot and does not** control HAGS. It never reads, writes or
...
- **Frame pacing**: the capture thread, the video render thread and the
  encoder thread all consume GPU resources; unnecessary GPUâ†’CPU readbacks
  and stalls are the main source of capture-side stutter.

## What obs-lite does to minimize overhead

obs-lite does not invent magic: it simply avoids unnecessary work and
synchronization points in the recording pipeline:

1. **GPU-resident encoding** â€” the preferred NVENC encoder
   (`obs_nvenc_h264_tex`) consumes textures directly on the GPU. No
   GPUâ†’CPU readback of the video frame is performed for encoding; the only
   readback is the small texture draw required for the preview window, and
   the preview can be disabled entirely.
2. **No double rendering** â€” the single scene is rendered once per frame
   by the libobs video thread. There are no scene collections, transitions,
   or extra composition passes.
3. **Preview can be disabled** â€” when the preview is hidden/minimized/
   disabled, `obs_display_set_enabled(false)` stops preview rendering
   entirely while capture and recording continue.
4. **No per-frame busy waits** â€” capture threads block on GPU events with
   proper waits; the libobs video thread paces itself to the configured FPS
   rather than spinning.
5. **Encoded-packet replay buffer** â€” the replay buffer never re-encodes or
   re-renders old frames, so it adds no GPU load while running.
6. **Decoupled disk I/O** â€” the `ffmpeg-mux` helper process performs
   container muxing and file writes, so temporary disk stalls do not block
   the GPU capture pipeline.
7. **Mature upstream capture code** â€” win-capture's game capture hook and
   DXGI duplicator code are the same code OBS Studio ships and are
   battle-tested across many HAGS-enabled systems. obs-lite does not
   reimplement them.
...
1. **Check the stats line** in the obs-lite window (dropped frames,
   recording FPS, encoder).
2. **Check the log** at `%APPDATA%\obs-lite\logs\obs-lite-*.log` for
...
5. Try the **Performance** encoder preset (Settings â†’ Video â†’ Preset).
